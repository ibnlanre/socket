...
const configuration = { baseURL: "wss://example.com", url: "/ws", disableCache: true };

describe("connection identity and parameter preparation", () => {
  it("uses normalized output for both pooling and the connection URL", () => {
    const client = new SocketClient({ ...configuration, paramsSchema: z.object({ room: z.string().toLowerCase() }) });
  ...
  it("ignores object order and encodes query values exactly once", () => {
    const client = new SocketClient<unknown, never, { a: string; b: string }>({ ...configuration });
    const socket = client.get({ a: "hello world", b: "&+/é" });
    expect(client.get({ b: "&+/é", a: "hello world" })).toBe(socket);
    expect(new URL(socket.path, configuration.baseURL).searchParams.get("b")).toBe("&+/é");
    client.closeAll();
  });
  ...
  it("deduplicates preparation while cancelling callers independently", async () => {
    ...
    const paramsSchema: StandardSchemaV1<string, { room: string }> = { "~standard": { version: 1, vendor: "test", validate } };
    ...
    resolve(); const prepared = await pending;
  ...
  it("infers schema input and output independently", async () => {
    ...
    expectTypeOf(client.get(prepared).value).toEqualTypeOf<number | undefined>();
    expectTypeOf(client.get(prepared).send).parameter(0).toEqualTypeOf<string>();
    client.closeAll();
  });

  it("exposes pending and error states without starting async work during render", async () => {
    const client = new SocketClient({ ...configuration, paramsSchema: z.string().refine(async (room) => room !== "bad").transform((room) => ({ room })) });
    const { result, rerender, unmount } = renderHook(({ room }) => client.usePreparedParams(room), { initialProps: { room: "first" } });
    expect(result.current.isPending).toBe(true);
    await waitFor(() => expect(result.current.params?.params.room).toBe("first"));
    rerender({ room: "bad" });
    expect(result.current.params).toBeUndefined();
    await waitFor(() => expect(result.current.error).toBeInstanceOf(Error));
    unmount(); client.closeAll();
  });

  it("selects data without rendering for unrelated snapshot changes", () => {
    const client = new SocketClient<number>({ ...configuration, placeholderData: 1 });
    let renders = 0;
    const hook = renderHook(() => { renders++; return client.useValue({ enabled: false }); });
    const before = renders;
    act(() => client.get().close());
    expect(hook.result.current).toBe(1);
    expect(renders).toBe(before);
    hook.unmount(); client.closeAll();
  });
});
